...
const CreateContentDialog: React.FC<CreateContentDialogProps> = ({
  ...
  const [newContent, setNewContent] = useState<ContentFormData>({
    title: "",
    description: "",
    lesson: [{ text: "", audio: "", video: "", inputMode: "text" }],
    file_path: [],
    file_type: "document",
  ...
  const addLessonItem = () => {
    setNewContent((prev) => ({
      ...prev,
      lesson: [...prev.lesson, { text: "", audio: "", video: "", inputMode: "text" }],
    }));
  };
  ...
  const updateLessonItem = (
    index: number,
    field: "text" | "audio" | "video" | "inputMode",
    value: string | 'text' | 'math'
  ) => {
    setNewContent((prev) => ({
      ...prev,
  ...
  // Initialize mathfields when mode changes
  useEffect(() => {
    newContent.lesson.forEach((item, index) => {
      const container = document.getElementById(`math-container-${index}`);
      if (item.inputMode === 'math' && container && !mathfieldRefs.current[index]) {
        ...
        mathfield.addEventListener('input', () => {
          updateLessonItem(index, "text", mathfield.value);
        });
        
        mathfieldRefs.current[index] = mathfield;
      } 
      else if (item.inputMode === 'text' && mathfieldRefs.current[index]) {
        mathfieldRefs.current[index]?.remove();
        mathfieldRefs.current[index] = null;
      }
    });

    return () => {
      // Cleanup all mathfields on unmount
      mathfieldRefs.current.forEach(mathfield => {
        mathfield?.remove();
      });
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Button
                          ...
                          onClick={() => 
                            updateLessonItem(
                              index, 
                              "inputMode", 
                              lessonItem.inputMode === "text" ? "math" : "text"
                            )
                          }
                          className="text-xs"
                        >
                          {lessonItem.inputMode === "text" 
                      ...
                      {lessonItem.inputMode === "text" ? (
                        ...
                        <div 
                          id={`math-container-${index}`}
                          className="border-2 border-gray-200 rounded-md p-2 min-h-[150px] bg-white"
                        />
                      )}
                    </div>
